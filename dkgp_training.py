...
# Load baseline-only CSV (one row per subject, Time=0) generated by accord_data.py.
# Using this file instead of accord_test_data ensures the full ACCORD population
...
for x_str in accord_baseline_data['X']:
    a = x_str.strip('][').split(', ')
    accord_baseline_features.append(np.array([float(v) for v in a]))

accord_baseline_np = np.array(accord_baseline_features)  # shape: [n_subjects, n_features]
logger.info(f"ACCORD baseline feature matrix shape: {accord_baseline_np.shape}")
