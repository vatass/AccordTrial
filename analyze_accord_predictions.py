...
# ---------------------------------------------------------------------------
# Fig 1: Population spaghetti + mean + observed BAG
# ---------------------------------------------------------------------------
...
for ptid, subj in ensemble.groupby('PTID'):
    ...
    ax.plot(subj['time_months'], subj['predicted'],
            color=TRAJ_COLOR, lw=0.6, alpha=0.12, zorder=1)
...
ax.fill_between(pop['time_months'], pop['ci_lo'], pop['ci_hi'],
                alpha=0.28, color='gray', zorder=2, label='95% CI (population mean)')
...
if obs_mask.any():
    ...
    ax.errorbar(obs_agg['time_months'], obs_agg['mean'],
                yerr=1.96 * obs_agg['se'], fmt='D', color='crimson',
                ms=9, lw=2, capsize=6,
                label='Observed BAG (mean ± 95% CI)', zorder=5)

ax.set_xlabel('Time (months)', fontsize=13)
ax.set_ylabel('BAG (years)', fontsize=13)
ax.set_title('ACCORD: Individual and Population BAG Trajectories — 8-Year Prediction', fontsize=14)
...
for ax, sex_label in zip(axes, ['Male', 'Female']):
    ...
    for ptid, subj_s in sub.groupby('PTID'):
        ...
        ax.plot(subj_s['time_months'], subj_s['predicted'],
    ...
    ax.fill_between(mean_bounds['time_months'],
                    ...
                    alpha=0.10, color=TRAJ_COLOR, label='Mean pred. interval', zorder=2)
    ...
    if len(real_s) > 0:
        ...
        ax.errorbar(obs_s['time_months'], obs_s['mean'],
                    yerr=1.96 * obs_s['se'], fmt='D', color='crimson',
                    ms=8, lw=2, capsize=5, label='Observed BAG')

    ax.set_title(f'{sex_label}  (n={sub["PTID"].nunique()})', fontsize=13)
...
plt.tight_layout()
fig.savefig(os.path.join(args.output_dir, 'fig2_sex_stratified.png'), dpi=150)
plt.close(fig)
...
for ax, ptid in zip(axes_flat, chosen):
    ...
    ax.plot(subj['time_months'], subj['predicted'], '-', color=TRAJ_COLOR, lw=2,
            label='Predicted')
    ax.fill_between(subj['time_months'], subj['lower_bound'], subj['upper_bound'],
                    alpha=0.25, color=TRAJ_COLOR, label='90% PI')
    ...
    ax.set_title(f'{sex_lbl}, {age_str}', fontsize=8.5)
...
axes_flat[min(len(chosen) - 1, len(axes_flat) - 1)].legend(
...
if obs_mask.any() and len(obs_tps) > 0:
    ...
    for ax, tp in zip(axes[0], obs_tps):
        ...
        r, pval = stats.pearsonr(tp_data['ground_truth'], tp_data['predicted'])
        lo = min(tp_data['ground_truth'].min(), tp_data['predicted'].min()) - 2
        hi = max(tp_data['ground_truth'].max(), tp_data['predicted'].max()) + 2
        ...
        ax.set_xlabel('Observed BAG (yr)', fontsize=11)
        ax.set_ylabel('Predicted BAG (yr)', fontsize=11)
        ax.set_title(f't={tp}m\nr={r:.3f}, MAE={mae_tp:.2f} yr\n(n={len(tp_data)})',
                     fontsize=11)
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)
...
plt.tight_layout()
fig.savefig(os.path.join(args.output_dir, 'fig5_distribution_over_time.png'), dpi=150)
plt.close(fig)
